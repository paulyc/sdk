...
bool LocalNode::excluded(const string& name) const
{
    ...
    LocalPath localPath;
    auto* node = this;
    auto& separator = sync->client->fsaccess->localseparator;
    ...
    else
    {
        ...
        const LocalPath localName =
          LocalPath::fromName(name, fsAccess, sync->mFilesystemType);
        ...
        {
            path = LocalPath::fromLocalname("static rule");
        }
    }
...
{
    return children.count(&Sync::IGNOREFILE_LOCALNAME);
}
